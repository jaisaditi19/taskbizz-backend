 ...
 */
async function resolveRecipient(tenant, occ) {
    if (!occ.clientId)
        return null;
    try {
...
async function buildEmailForOccurrence({ orgId, tenant, corePrisma, occurrence, }) {
    ...
    await Promise.all(allAttachments.map(async (att) => {
        try {
            ...
            console.warn(`Failed to download attachment ${att.id || att.key}:`, err?.message || err);
            try {
                const url = await (0, spacesUtils_1.getCachedFileUrlFromSpaces)(att.key, orgId);
    ...
    try {
        ...
        if (assigneeIds.length > 0) {
            ...
            try {
                ...
                console.warn("core.user.findMany failed:", e?.message ?? e);
            }
            if (!coreUsers || coreUsers.length === 0) {
                try {
                    ...
                    coreUsers = dirs.map((d) => ({ id: d.userId, name: d.name }));
                }
                catch (e) {
                    console.warn("tenant.directoryUser.findMany failed:", e?.message ?? e);
                }
            }
        ...
        console.warn("Failed to fetch assignee profiles:", e?.message ?? e);
    }
...
async function processTenant(org) {
    const dbUrl = (0, dbUrl_1.getOrgDbUrl)(org.dbName);
    ...
    try {
        ...
        if (!occurrences.length)
            return;
        const limit = (0, p_limit_1.default)(EMAIL_CONCURRENCY);
        await Promise.all(occurrences.map((occ) => limit(async () => {
            const lockKey = `occ-start-mail:${occ.id}`;
            const token = await (0, locks_1.acquireLock)(lockKey, 60);
            if (!token)
                return;
            try {
                ...
                if ((fresh.clientMailSendCount ?? 0) >= 3) {
                    await tenant.emailLog.create({
                        data: {
                            ...
                            taskId: occ.taskId,
                            recipient: "unknown",
                            subject: `Task "${occ.title ?? occ.task?.title ?? "Task"}" start - not sent`,
                            status: "FAILED",
                ...
                if (!recipient) {
                    await tenant.emailLog.create({
                        data: {
                            ...
                            subject: `Task "${occ.title ?? occ.task?.title ?? "Task"}" start - not sent`,
                            status: "FAILED",
                            error: "Client communication disabled or missing client email",
                        },
                    });
                    return;
                }
                // Build email content identical to sendTaskToClient
                const { subject, text, html, emailAttachments } = await buildEmailForOccurrence({
                    orgId: org.id,
                    tenant,
                    corePrisma: core,
                    occurrence: occ,
                ...
                console.error(`org=${org.id} occ=${occ.id} failed`, err?.message ?? err);
                try {
                    await tenant.emailLog.create({
                        data: {
                            occurrenceId: occ.id,
                ...
                await (0, locks_1.releaseLock)(lockKey, token).catch(() => { });
            }
        })));
    }
    finally {
...
async function sweepAllTenants() {
    ...
    const limit = (0, p_limit_1.default)(TENANT_CONCURRENCY);
    await Promise.all(validOrgs.map((o) => limit(() => processTenant(o))));
}
// Run once a day at 12:00 PM IST
const task = node_cron_1.default.schedule("0 12 * * *", async () => {
    try {
