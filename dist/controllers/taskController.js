...
exports.sendTaskToClient = sendTaskToClient;
exports.getOccurrenceDocs = getOccurrenceDocs;
exports.deleteTaskAttachment = deleteTaskAttachment;
exports.bulkUpdateOccurrences = bulkUpdateOccurrences;
exports.copyOccurrenceAttachments = copyOccurrenceAttachments;
exports.deleteOccurrenceAttachment = deleteOccurrenceAttachment;
exports.notifyOccurrenceAssigneesAndClient = notifyOccurrenceAssigneesAndClient;
exports.sendTaskCreationNotifications = sendTaskCreationNotifications;
exports.listWeeklyOffTasksForEmployee = listWeeklyOffTasksForEmployee;
...
const taskRecurrenceService_2 = require("../services/taskRecurrenceService");
...
const featureGates_1 = require("../middlewares/featureGates");
...
async function syncTaskAssignees(orgPrisma, taskId, userIds) {
 ...
 */
async function canEmployeeEditOccurrence(orgPrisma, corePrisma, req, occurrenceId) {
...
exports.uploadTaskFiles = (0, multer_1.default)({ storage }).array("attachments");
...
function parseDateInput(input) {
    ...
    if (typeof input === "string") {
        ...
        const d = luxon_1.DateTime.fromFormat(input, "d/M/yyyy", { zone: "utc" });
        if (d.isValid)
            return d.toJSDate();
        // Try ISO fallback
        const iso = luxon_1.DateTime.fromISO(input, { zone: "utc" });
        if (iso.isValid)
            return iso.toJSDate();
    }
    return null;
...
exports.createTask = [
    ...
    async (req, res) => {
        try {
            ...
            const { clientId, projectId, title, description, startDate, dueDate, assignedToId, // legacy single (fallback only)
            assignedToIds: rawAssignedToIds, priority, remarks, status, recurrenceRule, recurrenceEndDate, repeatNature, // kept for compatibility
            customValues: rawCustomValues = [], } = req.body;
            ...
            const assignedToIds = normalizeAssignedToIds(rawAssignedToIds ?? normalizedAssignedToId);
            ...
            if (!Array.isArray(customValues))
                customValues = [];
            const simpleRule = typeof recurrenceRule === "string" && recurrenceRule.trim() !== ""
                ? recurrenceRule.trim()
                : typeof repeatNature === "string" && repeatNature.trim() !== ""
                    ? repeatNature.trim()
                    : null;
            if (simpleRule && !(0, rrule_1.parseRecurrenceRule)(simpleRule)) {
                return res
                    .status(400)
                    .json({ message: `Invalid recurrenceRule: "${simpleRule}"` });
            }
            // Upload attachments
            let attachmentKeys = [];
            if (req.files && Array.isArray(req.files)) {
                attachmentKeys = await Promise.all(req.files.map((file) => (0, spacesUtils_1.uploadFileToSpaces)(file, req.user.orgId)));
            }
            ...
            const createdTask = await (0, taskRecurrenceService_1.createTaskWithSchedule)(orgPrisma, {
                clientId: normalizedClientId,
                projectId: normalizedProjectId,
                title,
                description,
                startDate: luxon_1.DateTime.fromISO(startDate, { zone: "utc" }).toJSDate(),
                dueDate: luxon_1.DateTime.fromISO(dueDate, { zone: "utc" }).toJSDate(),
                assignedToId: null,
                priority,
                remarks,
                status,
                recurrenceRule: simpleRule, // still parseRecurrenceRule-compatible; see note below
                recurrenceEndDate: recurrenceEndDate
                    ? luxon_1.DateTime.fromISO(recurrenceEndDate, { zone: "utc" }).toJSDate()
                    : null,
            ...
            await syncTaskAssignees(orgPrisma, createdTask.id, assignedToIds);
            if (assignedToIds.length > 0) {
                ...
                if (occIds.length) {
                    await orgPrisma.$transaction(async (tx) => {
                        ...
                        const rows = occIds.flatMap((occId) => assignedToIds.map((userId) => ({ occurrenceId: occId, userId })));
                        if (rows.length) {
            ...
            const attachmentsWithUrls = await Promise.all(createdTask.attachments.map(async (att) => ({
                id: att.id,
                key: att.key,
            ...
            await (0, notify_1.notifyCounterparties)({
                orgPrisma,
                corePrisma: (0, container_1.getCorePrisma)(),
                io: req.io,
                orgId: String(req.user.orgId),
                actor: { id: String(req.user.id), role: req.user.role },
                assigneeIds: (taskWithAssignees?.assignees ?? []).map((a) => String(a.userId)),
                payload: {
                    type: "TASK_CREATED",
                    title: "Task created",
 ...
 */
async function completeOccurrence(req, res) {
    try {
        ...
        // Permission
        const canEdit = await canEmployeeEditOccurrence(orgPrisma, prisma, req, occurrenceId);
        if (!canEdit) {
            return res
                .status(403)
 ...
 */
exports.updateOccurrence = [
    exports.uploadTaskFiles,
    featureGates_1.requireDocsFeatureIfDocOps,
    async (req, res) => {
        try {
            const occurrenceId = req.params.id;
            const { title, description, startDate, dueDate, assignedToId, assignedToIds: rawAssignedToIds, priority, remarks, status, customValues = [], } = req.body;
            const orgPrisma = await resolveOrgPrisma(req);
            const prisma = await (0, container_1.getCorePrisma)();
            // Permission
            const canEdit = await canEmployeeEditOccurrence(orgPrisma, prisma, req, occurrenceId);
            ...
            const normalizeAssignedToIds = (v) => {
                if (v === undefined || v === null)
                    return undefined; // <--- important: undefined = "not provided"
                if (Array.isArray(v))
            ...
            const assignedToIdsNew = normalizeAssignedToIds(rawAssignedToIds !== undefined
                ? rawAssignedToIds
            ...
            let occurrenceAttachmentsToRemove = [];
            ...
            if (occurrenceAttachmentsToRemove.length > 0) {
                for (const attachmentId of occurrenceAttachmentsToRemove) {
                    ...
                    if (attachment) {
                        try {
                            await (0, spacesUtils_1.deleteFileFromSpaces)(attachment.key);
            ...
            let newAttachmentKeys = [];
            if (req.files && Array.isArray(req.files)) {
                newAttachmentKeys = await Promise.all(req.files.map(async (file) => (0, spacesUtils_1.uploadFileToSpaces)(file, req.user.orgId)));
            ...
            const updated = await orgPrisma.taskOccurrence.update({
                ...
                data: {
                    ...
                    description,
                    startDate: startDate
                        ? luxon_1.DateTime.fromISO(startDate, { zone: "utc" }).toJSDate()
                        : undefined,
                    dueDate: dueDate
                        ? luxon_1.DateTime.fromISO(dueDate, { zone: "utc" }).toJSDate()
                        : undefined,
                    assignedToId: assignedToIdsNew && assignedToIdsNew.length > 0
                        ? assignedToIdsNew[0]
                        : assignedToId ?? undefined,
                    priority,
                    remarks,
            ...
            const normalizedCustomValues = finalTask?.customValues.map((cv) => ({
                id: cv.id,
                fieldId: cv.fieldId,
            ...
            const occurrenceAttachmentsWithUrls = await Promise.all(updated.attachments.map(async (att) => ({
                id: att.id,
                key: att.key,
                url: await (0, spacesUtils_1.getCachedFileUrlFromSpaces)(att.key, req.user.orgId),
                type: "occurrence",
            ...
            const taskAttachmentsWithUrls = await Promise.all(updated.task.attachments.map(async (att) => ({
                id: att.id,
                key: att.key,
                url: await (0, spacesUtils_1.getCachedFileUrlFromSpaces)(att.key, req.user.orgId),
                type: "task",
            ...
            res.json({
                ...
                assignedToIds: (await orgPrisma.taskOccurrenceAssignee.findMany({
                    where: { occurrenceId },
                    select: { userId: true },
                })).map((r) => r.userId),
                taskAssignedToIds: finalTask?.assignees?.map((a) => a.userId) ?? [],
                debug: {
                    occurrenceId,
            ...
            const occurrenceAssignees = await orgPrisma.taskOccurrenceAssignee.findMany({
                where: { occurrenceId },
                select: { userId: true },
            });
            await (0, notify_1.notifyCounterparties)({
                ...
                orgId: req.user.orgId,
                actor: { id: req.user.id, role: req.user.role },
                assigneeIds: occurrenceAssignees.map((a) => a.userId),
                payload: {
                    type: "OCCURRENCE_COMPLETED",
                    title: "Task completed",
                    body: `"${updated.title ?? "Untitled"}" marked complete.`,
...
exports.updateTask = [
    ...
    async (req, res) => {
        try {
            ...
            const { clientId, projectId, title, description, startDate, dueDate, assignedToId, // legacy single (fallback)
            assignedToIds: rawAssignedToIds, priority, remarks, status, recurrenceRule, recurrenceEndDate, repeatNature, 
            // updateFutureOccurrences = true,
            scope = "ALL", anchorOccurrenceId, customValues: rawCustomValues = [], } = req.body;
            ...
            try {
                if (req.files &&
                    ...
                    req.files.length) {
                    newTaskAttachmentKeys = await Promise.all(req.files.map((file) => (0, spacesUtils_1.uploadFileToSpaces)(file, req.user.orgId)));
            ...
            // Only include a field in scheduleChange if the client actually sent
            // it — this is what prevents ever silently falling back to the old
            // date/rule (that was the root cause of the original bug).
            const scheduleChange = {};
            if (startDate !== undefined) {
                const dt = luxon_1.DateTime.fromISO(startDate, { zone: "utc" });
                if (dt.isValid)
                    scheduleChange.startDate = dt.toJSDate();
            }
            if (dueDate !== undefined) {
                const dt = luxon_1.DateTime.fromISO(dueDate, { zone: "utc" });
                if (dt.isValid)
                    scheduleChange.dueDate = dt.toJSDate();
            }
            if (recurrenceEndDate !== undefined) {
                scheduleChange.recurrenceEndDate = recurrenceEndDate
                    ? luxon_1.DateTime.fromISO(recurrenceEndDate, { zone: "utc" }).toJSDate()
                    : null;
            }
            if (recurrenceRule !== undefined || repeatNature !== undefined) {
                scheduleChange.recurrenceRule =
            ...
            const scheduleTouched = Object.keys(scheduleChange).length > 0;
            const templatePatch = {
                title,
                description,
                priority,
            ...
            // 5) Apply the update via the recurrence-aware service.
            const serviceResult = scope === "THIS_AND_FOLLOWING"
                ? await (0, taskRecurrenceService_2.splitSeries)(orgPrisma, taskId, anchorOccurrenceId, templatePatch, scheduleTouched ? scheduleChange : undefined)
                : await (0, taskRecurrenceService_2.updateTaskAllScope)(orgPrisma, taskId, templatePatch, scheduleTouched ? scheduleChange : undefined);
            const respondingTaskId = scope === "THIS_AND_FOLLOWING"
                ? serviceResult.oldTask.id
                : taskId;
            if (rawAssignedToIds !== undefined || assignedToId !== undefined) {
                await syncTaskAssignees(orgPrisma, respondingTaskId, assignedToIds);
                if (scope === "THIS_AND_FOLLOWING") {
                    await syncTaskAssignees(orgPrisma, serviceResult.newTask.id, assignedToIds);
                }
            }
            // Custom values are independent of recurrence scope. Guarded by
            // .length so an update that doesn't touch custom fields no longer
            // silently wipes them (the old `if (Array.isArray(customValues))`
            // was always true since it defaults to [], so every update — even
            // ones that never mentioned customValues — cleared them).
            ...
            // Project/client are referential-integrity fields, not historical
            // facts — sync onto every occurrence, including completed/cancelled
            // ones, unlike other content fields (matches your original "critical
            // fix" intent).
            ...
            if (scheduleTouched) {
                ...
                if (firstOccurrence) {
                    await (0, notify_1.notifyCounterparties)({
                        ...
                        payload: {
                            type: "TASK_UPDATED",
                            title: `Task updated: ${title || existingTask.title}`,
            ...
            const responseMessage = scheduleTouched
            ...
            res.json({
                ...
                taskAssignedToIds: [],
                // Only present for scope === "THIS_AND_FOLLOWING" — the id of the
                // new series that starts at the split point.
                newSeriesTaskId: scope === "THIS_AND_FOLLOWING"
                    ? serviceResult.newTask.id
                    : undefined,
                debug: {
                    scope,
                    regeneratedOccurrences: scheduleTouched,
                    changes: {
                        recurrenceRule: scheduleChange.recurrenceRule !== undefined,
                        startDate: scheduleChange.startDate !== undefined,
                        dueDate: scheduleChange.dueDate !== undefined,
            ...
            if (scheduleTouched) {
                req.io
                    .to(`org:${orgId}`)
                    .emit("occurrence:refresh", { orgId, taskId: respondingTaskId });
                if (scope === "THIS_AND_FOLLOWING") {
                    req.io.to(`org:${orgId}`).emit("occurrence:refresh", {
                        orgId,
                        taskId: serviceResult.newTask.id,
                    });
                }
            }
        }
    ...
    try {
        ...
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
          ...
          <div style="background-color: #f8f9fa; padding: 20px; border-radius: 8px; margin: 20px 0;">
            ...
            <p><strong>Status changed from:</strong> ${previousStatus || "Unknown"} → <strong>${status}</strong></p>
            <p><strong>Changed by:</strong> ${actorName}</p>
            ${updated.task.description
                ? `<p><strong>Description:</strong> ${updated.task.description}</p>`
                : ""}
            ${updated.dueDate
                ? `<p><strong>Due Date:</strong> ${new Date(updated.dueDate).toLocaleDateString()}</p>`
                : ""}
          </div>
      ...
      `;
        ...
        req.io.to(`org:${orgId}`).emit("occurrence:status", {
            ...
            occurrence: {
                ...
                isCompleted: updated.isCompleted ?? false,
                assignedToId: updated.assignedToId,
                assignedToIds: updated.assignees?.map((a) => a.userId) || [],
                priority: updated.priority,
                clientId: updated.clientId,
                projectId: updated.projectId,
                startDate: updated.startDate,
        ...
        try {
            const occurrenceAssignees = updated.assignees?.map((a) => a.userId) || [];
            await (0, notify_1.notifyCounterparties)({
                orgPrisma,
                corePrisma: prisma,
                io: req.io,
    ...
    try {
        ...
        for (const occurrence of occurrences) {
            for (const attachment of occurrence.attachments) {
                try {
                    ...
                    console.error(`Failed to delete occurrence attachment ${attachment.key}:`, error);
                }
            }
        }
        ...
        for (const attachment of task.attachments) {
            try {
                ...
                console.error(`Failed to delete task attachment ${attachment.key}:`, error);
            }
        }
...
const bulkUploadTasks = async (req, res) => {
    try {
        ...
        for (const row of tasks) {
            try {
                const { title, description, startDate, dueDate, priority, status, assignedToId, clientId, projectId, remarks, } = row;
                if (!title || !startDate || !dueDate) {
                    results.push({
                        ok: false,
...
exports.bulkUploadTasks = bulkUploadTasks;
 ...
 */
async function sendWhatsAppTemplate(toE164, clientName) {
 ...
 */
async function sendWhatsAppText(toE164, textBody) {
...
async function sendTaskToClient(req, res) {
    try {
        ...
        if (!occurrenceId && !taskId) {
            console.warn("sendTaskToClient called without occurrenceId or taskId. Request body:", JSON.stringify(body));
        }
        ...
        if (occurrenceId) {
            ...
            return res.status(400).json({
                error: "Missing identifier. Provide occurrenceId, or taskId + occurrenceIndex.",
            });
        }
        if (!occurrence) {
            return res.status(404).json({
                error: "Task occurrence not found. Confirm occurrenceId, or taskId + occurrenceIndex, or valid taskId.",
            });
        }
        ...
        await Promise.all(allAttachments.map(async (att) => {
            try {
                ...
                console.warn(`Failed to download attachment ${att.id || att.key}:`, downloadErr?.message || downloadErr);
                try {
                    const url = await (0, spacesUtils_1.getCachedFileUrlFromSpaces)(att.key, req.user.orgId);
                    fallbackUrls.push(url);
                }
        ...
        if (Array.isArray(frontendAssignees) && frontendAssignees.length > 0) {
            ...
            try {
                const occAssignees = (occurrence.assignees ?? []);
                if (Array.isArray(occAssignees) && occAssignees.length > 0) {
                    ...
                    const taskAssignees = (occurrence.task?.assignees ?? []);
                        ...
                        normalizedAssignees = taskAssignees
                            ...
                            id: u?.userId
                                ...
                                : u?.id
                                    ? String(u.id)
                                    : undefined,
                            name: u && (u.name || u.fullName || u.displayName)
                                ? String(u.name || u.fullName || u.displayName)
                                : undefined,
                            email: u && (u.email || u.userEmail)
        ...
        const assigneeLinesText = normalizedAssignees.length === 0
            ? "Unassigned"
            : normalizedAssignees
                .map((a) => {
        ...
        const assigneeNameEmailHtml = normalizedAssignees.length === 0
            ? "—"
            : normalizedAssignees
                .map((a) => {
                const name = a.name ?? `#${a.id ?? "?"}`;
        ...
        const contactNumbersArr = normalizedAssignees
            .map((a) => a.phone)
            .filter(Boolean);
        const contactNumbersText = contactNumbersArr.length > 0 ? contactNumbersArr.join("; ") : "—";
        ...
        try {
            ...
            if (assigneeIds.length > 0) {
                ...
                try {
                    ...
                    console.warn("prisma.user.findMany failed:", e?.message ?? e);
                }
                ...
                if ((!coreUsers || coreUsers.length === 0) &&
                    ...
                    try {
                        ...
                        coreUsers = dirs.map((d) => ({
                            id: d.userId ?? d.user_id,
                            name: d.name,
                        }));
                    }
                    catch (e) {
                        console.warn("prisma.directoryUser.findMany failed:", e?.message ?? e);
                    }
                }
                // Build lookup map
                const usersById = (coreUsers || []).reduce((acc, u) => {
                ...
                if (unresolved.length > 0) {
                    console.debug("[sendTaskToClient] unresolved assignee ids (no profile found):", unresolved);
                }
            }
        }
        catch (e) {
            console.warn("Failed to fetch assignee profiles from core DB:", e?.message ?? e);
        }
        // ---------- END INSERT ----------
        // Format due date
        ...
        const subject = `${occurrence.title ?? "Task"} is "${occurrence.status ?? "Open"}"`;
        // Plain text body
        const textParts = [
            organizationName,
            "",
        ...
        textParts.push("Please do not reply to this email. This mailbox is not monitored.");
        const text = textParts.join("\n");
        // HTML body — minimal, requested fields
        const attachmentsHtmlSection = downloadedAttachments.length > 0
...
const getDashboard = async (req, res) => {
    try {
        ...
        const user = req.user;
        /* ================= CACHE ================= */
        const cacheKey = (0, cache_1.orgKey)(orgId, "dashboard", `u=${user.id}:s=${start}:e=${end}`);
        ...
        const isOpen = (t) => (t.status || t.task?.status || "").toUpperCase() === "OPEN";
        const toDateSafe = (v) => {
            if (!v)
                return null;
        ...
        const assignedTasks = occurrences.filter((t) => t.assignedToId).length;
        ...
        for (const t of occurrences) {
            ...
            if (t.assignedToId) {
                if (!byUser.has(t.assignedToId))
                    byUser.set(t.assignedToId, []);
        ...
        const weeklyCompletionRate = createdInWindow.length > 0
            ? Math.round((completedInWindow.length / createdInWindow.length) * 100)
            : 0;
        /* ================= WEEK DUE ================= */
        const weekDue = selectedWeekStart && selectedWeekEnd
            ? occurrences.filter((t) => {
                if (isTaskCompleted(t))
                    return false;
                const due = toDateSafe(t.dueDate);
                return due
            ...
            : occurrences.filter((t) => !isTaskCompleted(t) && t.dueDate)
                .length;
        /* ================= PROJECT PROGRESS ================= */
        ...
        if (isManager) {
            const involvedIds = new Set(occurrences.map((t) => t.assignedToId).filter(Boolean));
        ...
        /* ================= TASK DISTRIBUTION ================= */
        const counts = rawOccurrences.reduce((acc, t) => {
            const st = (t.status ||
                t.task?.status ||
        ...
        const getLastActivityTime = (t) => {
            return (t.completedAt?.getTime?.() ??
                t.updatedAt?.getTime?.() ??
                t.createdAt?.getTime?.() ??
                0);
        };
        const recentTasks = occurrences
            ...
            .sort((a, b) => new Date(b.lastActivityAt).getTime() -
        ...
        const payload = {
            stats: {
                ...
                activeProjects: projectProgress.filter((p) => p.totalTasks > p.completedTasks).length,
...
async function getOccurrenceDocs(req, res) {
    try {
        const occurrenceId = req.params.id;
        const orgId = req.user.orgId;
        const docsEnabled = !!req.subscriptionCtx?.plan?.features?.features?.documentManagement;
        // DEBUG logs
        // console.log(
        ...
        try {
            const [taskAttachmentCount, occAttachmentCount, occRow] = await Promise.all([
        ...
        const occAttachmentsWithUrls = await Promise.all(occRaw.map(async (att) => {
            try {
                const url = await (0, spacesUtils_1.getCachedFileUrlFromSpaces)(att.key, orgId);
                if (!url) {
                    console.warn("[DEBUG getOccurrenceDocs] presign returned falsy URL for", att.key);
                }
                return {
                    id: att.id,
                    key: att.key,
                ...
                console.error("[DEBUG getOccurrenceDocs] presign failed for", att.key, err);
                return {
                    id: att.id,
                    key: att.key,
                    url: null,
        ...
        const taskAttachmentsWithUrls = await Promise.all(taskRaw.map(async (att) => {
            try {
                const url = await (0, spacesUtils_1.getCachedFileUrlFromSpaces)(att.key, orgId);
                if (!url) {
                    console.warn("[DEBUG getOccurrenceDocs] presign returned falsy URL for", att.key);
...
async function deleteTaskAttachment(req, res) {
    try {
        ...
        try {
            await (0, spacesUtils_1.deleteFileFromSpaces)(att.key);
        }
        catch (s3Err) {
...
async function bulkUpdateOccurrences(req, res) {
    try {
        const prisma = (0, container_1.getCorePrisma)();
        const { ids, status, assignedToId, assignedToIds: rawAssignedToIds, } = req.body ?? {};
        ...
        const assignedToIds = rawAssignedToIds !== undefined
        ...
        await orgPrisma.$transaction(async (tx) => {
            ...
            if (assignedToIds !== undefined) {
                ...
                deletedAssigneeRows =
                    (delRes && typeof delRes.count === "number"
                        ? delRes.count
                ...
                if (assignedToIds.length > 0) {
                    ...
                    for (let i = 0; i < createPayload.length; i += CHUNK) {
                        ...
                        const added = (createRes && typeof createRes.count === "number"
                            ? createRes.count
        ...
        if (typeof status === "string") {
            ...
            const clientIds = Array.from(new Set(updatedOccurrences
                .map((o) => o.task?.clientId)
                .filter((cid) => !!cid)));
            const optInMap = {};
          ...
          ${occ.dueDate
                    ? `<p><strong>Due Date:</strong> ${new Date(occ.dueDate).toLocaleDateString()}</p>`
                    : ""}
        `.trim();
                try {
        ...
        await Promise.all(updatedOccurrences.map(async (occ) => {
            ...
            try {
                await (0, notify_1.notifyCounterparties)({
                    ...
                    orgId: req.user.orgId,
                    actor: { id: req.user.id, role: req.user.role },
                    assigneeIds: occAssignees.map((a) => a.userId),
                    payload: {
        ...
        req.io.to(`org:${orgId}`).emit("occurrence:bulk", {
            orgId,
            occurrences: updatedOccurrences.map((u) => ({
                id: u.id,
                taskId: u.taskId,
                title: u.title,
                status: u.status,
                assignedToId: u.assignedToId, // legacy scalar (optional)
                assignedToIds: u.assignees?.map((a) => a.userId) ?? [], // ✅ source of truth
                priority: u.priority,
                clientId: u.clientId,
 ...
 */
async function copyOccurrenceAttachments(req, res) {
    try {
        ...
        for (const m of mappings) {
            if (!m?.fromOccurrenceId || !m?.toOccurrenceId) {
                return res
                    .status(400)
                    .json({
                    error: "Each mapping must include fromOccurrenceId and toOccurrenceId",
                });
            }
        }
        const summary = [];
        ...
        await orgPrisma.$transaction(async (tx) => {
            ...
            for (const mapping of mappings) {
                ...
                const srcAttachments = await txAny.taskOccurrenceAttachment.findMany({
...
async function deleteOccurrenceAttachment(req, res) {
    try {
...
async function notifyOccurrenceAssigneesAndClient(orgPrisma, corePrisma, io, orgId, occurrenceId, emailSubject, // used for assignees
emailBody, // HTML body for assignees (not used for client now)
notificationPayload, opts = {}) {
    const { sendAssigneeEmails = false, sendInAppNotifications = false, respectClientOptIn = false, } = opts;
    ...
    const assigneeUserIds = occurrence.assignees?.map((a) => a.userId) || [];
    ...
    const assigneeNameEmailHtml = assigneeUsers.length > 0
        ? assigneeUsers
            .map((u) => {
    ...
    const contactNumbersText = assigneeUsers
        .map((u) => (u.phone || "").toString().trim())
        .filter(Boolean)
        .join(", ") || "—";
    ...
    const assigneeEmailPromises = sendAssigneeEmails
        ? assigneeUsers
            .filter((u) => !!u.email)
            .map(async (user) => {
            try {
    ...
    const notificationPromises = sendInAppNotifications
 ...
 */
async function sendTaskCreationNotifications(orgPrisma, corePrisma, io, orgId, taskId, occurrenceAssigneeIds, creatorId) {
    try {
        // Get task details
        const task = await orgPrisma.task.findUnique({
            where: { id: taskId },
        ...
        if (occurrenceAssigneeIds.length > 0) {
            ...
            for (const user of assigneeUsers) {
                try {
                    await (0, emailUtils_1.sendEmail)(user.email, `New Task Assigned: ${task.title}`, `<div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
              <h2>New Task Assignment</h2>
              <div style="background-color: #f8f9fa; padding: 20px; border-radius: 8px; margin: 20px 0;">
                <h3 style="margin-top: 0;">${task.title}</h3>
                <p><strong>Description:</strong> ${task.description || 'No description provided'}</p>
                <p><strong>Due Date:</strong> ${luxon_1.DateTime.fromJSDate(task.dueDate).toFormat("dd LLL yyyy")}</p>
        ...
        if (task.client && task.client.email && task.client.clientCommunication) {
            try {
                await (0, emailUtils_1.sendEmail)(task.client.email, `New Task Created: ${task.title}`, `<div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
...
async function listWeeklyOffTasksForEmployee(req, res) {
    try {
        // ✅ ORG DB (for tasks)
        const orgPrisma = await resolveOrgPrisma(req);
        const prisma = await (0, container_1.getCorePrisma)();
        ...
        const start = luxon_1.DateTime.utc(Number(year), 1, 1).startOf("day");
        const end = luxon_1.DateTime.utc(Number(year), 12, 31).endOf("day");
        /* 1️⃣ Weekly off — CORE DB */
        ...
        const offDays = weeklyOffs
            .map((w) => dayMap[w.day])
            .filter((d) => d !== undefined);
        /* 2️⃣ Build weekly-off dates */
        const weeklyOffDates = [];
        let cursor = start;
        while (cursor <= end) {
            if (offDays.includes(cursor.weekday % 7)) {
                weeklyOffDates.push(cursor.toJSDate());
            }
            cursor = cursor.plus({ days: 1 });
        }
        if (!weeklyOffDates.length) {
            return res.json([]);
        }
        /* 3️⃣ Task occurrences — ORG DB */
        const rows = await orgPrisma.taskOccurrence.findMany({
            where: {
                AND: [
                    {
        ...
        console.error("weekly-off tasks error", err);
        res.status(500).json({ message: "Failed to fetch weekly-off tasks" });
    }
}
