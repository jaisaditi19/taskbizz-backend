...
exports.bulkUpdateProjectHeads = exports.bulkUploadProjects = exports.deleteProject = exports.updateProject = exports.getProjects = exports.createProject = void 0;
...
exports.bulkUploadProjects = bulkUploadProjects;
/**
 ...
 */
const bulkUpdateProjectHeads = async (req, res) => {
    try {
        const corePrisma = (0, container_1.getCorePrisma)();
        if (!req.user)
            return res.status(401).json({ message: "Unauthorized" });
        ...
        if (!orgId)
            return res.status(400).json({ message: "Org ID required" });
        const prisma = await resolveOrgPrisma(req);
        const { ids, head, updates } = req.body;
        // -------- Mode B: pairwise updates --------
        if (Array.isArray(updates) && updates.length > 0) {
            // validate head ids (ignore nulls)
            const uniqueHeads = Array.from(new Set(updates.map((u) => u.head).filter((h) => typeof h === "string" && h.trim() !== "")));
            ...
            const settled = await Promise.allSettled(updates.map(({ id, head }) => prisma.project.update({
                where: { id },
                data: { head: head ?? null },
            })));
            const failed = [];
...
exports.bulkUpdateProjectHeads = bulkUpdateProjectHeads;
