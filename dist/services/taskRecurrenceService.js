...
exports.createTaskWithSchedule = createTaskWithSchedule;
exports.updateTaskAllScope = updateTaskAllScope;
exports.splitSeries = splitSeries;
exports.extendAllOccurrenceWindows = extendAllOccurrenceWindows;
const luxon_1 = require("luxon");
const recurrenceEngine_1 = require("../services/recurrenceEngine");
const DEFAULT_HORIZON_DAYS = 90;
function horizonWindowEnd(days) {
    return luxon_1.DateTime.now().setZone(recurrenceEngine_1.STEP_ZONE).plus({ days }).endOf("day").toUTC().toJSDate();
}
function singleOccurrencePayload(task) {
    return {
        title: task.title,
 ...
 */
async function createTaskWithSchedule(orgPrisma, createData, horizonDays = DEFAULT_HORIZON_DAYS) {
    const task = await orgPrisma.task.create({
        data: createData,
        include: { attachments: true, customValues: { include: { field: true } } },
    });
    if (task.isRecurring) {
        await (0, recurrenceEngine_1.withTaskLock)(orgPrisma, task.id, (tx, lockedTask) => (0, recurrenceEngine_1.syncOccurrenceWindow)(tx, lockedTask, horizonWindowEnd(horizonDays)));
    }
    else {
 ...
 */
async function updateTaskAllScope(orgPrisma, taskId, templatePatch, scheduleChange, horizonDays = DEFAULT_HORIZON_DAYS) {
    return (0, recurrenceEngine_1.withTaskLock)(orgPrisma, taskId, async (tx, currentTask) => {
        const now = new Date();
        const scheduleTouchesGeneration = !!scheduleChange &&
        ...
        if (resolvedStartDate !== undefined && resolvedDueDate === undefined) {
            const existingDurationMs = new Date(currentTask.dueDate).getTime() -
                new Date(currentTask.startDate).getTime();
            resolvedDueDate = new Date(resolvedStartDate.getTime() + existingDurationMs);
        }
        else if (resolvedDueDate !== undefined &&
        ...
        if (resolvedStartDate !== undefined &&
            ...
            throw new Error("Invalid schedule: dueDate cannot be before startDate for a recurring task.");
        }
        // 1) Template update
        const taskUpdateData = stripUndefined({
            ...
            assignedToId: templatePatch.assignedToIds?.[0] ?? templatePatch.assignedToId,
        });
        if (resolvedStartDate !== undefined)
            taskUpdateData.startDate = resolvedStartDate;
        if (resolvedDueDate !== undefined)
            taskUpdateData.dueDate = resolvedDueDate;
        ...
        if (templatePatch.assignedToIds) {
            ...
            const occIds = liveOccs.map((o) => o.id);
            if (occIds.length) {
                await tx.taskOccurrenceAssignee.deleteMany({
                    where: { occurrenceId: { in: occIds } },
                });
                const rows = occIds.flatMap((occId) => templatePatch.assignedToIds.map((uid) => ({
                    occurrenceId: occId,
                    userId: uid,
                })));
                if (rows.length)
        ...
        if (scheduleTouchesGeneration) {
            if (!freshTask.isRecurring) {
                ...
                await (0, recurrenceEngine_1.syncOccurrenceWindow)(tx, { ...freshTask, lastGeneratedUntil: null }, horizonWindowEnd(horizonDays));
            }
        }
 ...
 */
async function splitSeries(orgPrisma, taskId, anchorOccurrenceId, templatePatch, scheduleChange, horizonDays = DEFAULT_HORIZON_DAYS) {
    return (0, recurrenceEngine_1.withTaskLock)(orgPrisma, taskId, async (tx, oldTask) => {
        ...
        const splitDate = anchor.startDate;
        // 1) Cap the old series the day before the split point.
        const cappedEnd = luxon_1.DateTime.fromJSDate(splitDate)
            .setZone(recurrenceEngine_1.STEP_ZONE)
            .minus({ days: 1 })
            .endOf("day")
            .toUTC()
            .toJSDate();
        ...
        const newEnd = scheduleChange?.recurrenceEndDate !== undefined ? scheduleChange.recurrenceEndDate : oldTask.recurrenceEndDate;
        ...
        if (newTask.isRecurring) {
            await (0, recurrenceEngine_1.syncOccurrenceWindow)(tx, newTask, horizonWindowEnd(horizonDays));
        }
        else {
        ...
        if (templatePatch.assignedToIds?.length) {
            ...
            const rows = allOccs.flatMap((o) => templatePatch.assignedToIds.map((uid) => ({ occurrenceId: o.id, userId: uid })));
            if (rows.length)
                await tx.taskOccurrenceAssignee.createMany({ data: rows, skipDuplicates: true });
 ...
 */
async function extendAllOccurrenceWindows(orgPrisma, horizonDays = DEFAULT_HORIZON_DAYS) {
    ...
    for (const t of tasks) {
        try {
            const result = await (0, recurrenceEngine_1.withTaskLock)(orgPrisma, t.id, async (tx, task) => {
                if (!task.isRecurring)
                    return { created: 0, removedFutureStale: 0 }; // changed mid-run
                return (0, recurrenceEngine_1.syncOccurrenceWindow)(tx, task, targetWindowEnd);
            });
