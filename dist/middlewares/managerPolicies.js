...
exports.canManageWithinProject = void 0;
exports.resolveProjectIdFromReq = resolveProjectIdFromReq;
const container_1 = require("../di/container");
const S = (v) => (v == null ? null : String(v));
async function resolveProjectIdFromReq(req) {
    ...
    const directProjectId = p.projectId ??
        b.projectId ??
        q.projectId ??
        b?.task?.projectId ??
        b?.data?.projectId ??
    ...
    if (taskId) {
        ...
        if (t?.projectId)
            return S(t.projectId);
    }
    // 4) From occurrence id
    const occurrenceId = p.occurrenceId ??
        b.occurrenceId ??
        q.occurrenceId ??
    ...
    if (occurrenceId) {
        ...
        if (occ?.task?.projectId)
            return S(occ.task.projectId);
    }
    return null;
}
const canManageWithinProject = () => {
...
exports.canManageWithinProject = canManageWithinProject;
