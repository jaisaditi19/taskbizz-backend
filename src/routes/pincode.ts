...
const PINCODE_APIS = [
  {
    name: "IndianPincodes",
    url: (pincode: string) =>
      `https://www.indianpincodes.co.in/api/pincode/${pincode}`,
    transform: (data: any, pincode: string) => {
  ...
  },
  {
    ...
    url: (pincode: string) => `http://api.postalpincode.in/pincode/${pincode}`,
    transform: (data: any) => data, // Already in correct format
  },
  {
    name: "PostalPinCode_HTTPS",
    url: (pincode: string) => `https://api.postalpincode.in/pincode/${pincode}`,
    transform: (data: any) => data, // Already in correct format
  },
  {
    name: "Zippopotamus",
    url: (pincode: string) => `https://api.zippopotam.us/in/${pincode}`,
    transform: (data: any, pincode: string) => {
...
function makeHttpRequest(url: string, timeout: number = 10000): Promise<any> {
  return new Promise((resolve, reject) => {
    ...
    if (isHttps) {
      ...
      (options as any).secureProtocol = "TLSv1_2_method";
    }
...
async function fetchPincodeData(pincode: string): Promise<any> {
  const errors: any[] = [];
  ...
  throw new Error(
    `All pincode lookup strategies failed: ${JSON.stringify(errors, null, 2)}`
  );
}
