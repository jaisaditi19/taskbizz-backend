 ...
 */
function toE164IN(raw?: string | null): string | null {
  if (!raw) return null;
 ...
 */
export async function sendTaskBizzServiceUpdate(params: {
  phone: string;
  organizationName: string;
  clientName: string;
  work: string;
  ...
  if (hasEmptyParam) {
    const error = `[MyOperator] Template parameters cannot be empty. Params: ${JSON.stringify(
      paramStrings
    )}`;
    console.error(error);
    return { success: false, error };
...
const payload = {
  ...
  try {
    ...
    if (response.status >= 200 && response.status < 300) {
      ...
      console.error(
        "[MyOperator WA] ❌ Sent Payload:",
        JSON.stringify(payload, null, 2)
      );

      return {
        success: false,
        error: `API returned ${response.status}: ${JSON.stringify(
          response.data
        )}`,
      };
    }
 ...
 */
export async function testMyOperatorConfig(): Promise<void> {
  const requiredVars = {
