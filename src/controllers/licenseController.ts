...
function ensureDateOrThrow(
  ...
  if (!parsed) {
    if (required) throw new Error(`Valid ${fieldName} is required`);
    return null;
  }
      ...
      await orgPrisma.$transaction(async (tx: any) => {
        for (let j = 0; j < chunk.length; j++) {
          ...
          try {
            ...
            if (
              Array.isArray(row?.remindOffsets) &&
              row.remindOffsets.length > 0
            ) {
              remindOffsets = row.remindOffsets
                .map((n: any) => Number(n))
                .filter((n: number) => Number.isFinite(n) && n !== 0)
                .map((n: number) => -Math.abs(n)) // ensure negative
