...
export const getCalendarEntries = async (req: Request, res: Response) => {
  try {
    ...
    const where: any = {};
    if (startParam) where.start = { gte: startParam };
    if (endParam) where.start = { ...(where.start ?? {}), lt: endParam };
...
export const getCalendarEntry = async (req: Request, res: Response) => {
  try {
    const orgPrisma = await resolveOrgPrisma(req);
    const user = req.user as any;
    const { id } = req.params;
...
export const updateCalendarEntry = async (req: Request, res: Response) => {
  try {
    ...
    if (
      (interval != null || until != null || count != null) &&
      !freq &&
      existing.freq == null
    ) {
    ...
    const endDt =
      end === undefined ? undefined : end === null ? null : parseISO(end);
    const untilDt =
      until === undefined ? undefined : until === null ? null : parseISO(until);

    if (startDt === null)
      return res.status(400).json({ error: "Invalid start" });
    if (endDt === null) return res.status(400).json({ error: "Invalid end" });
    if (untilDt === null)
      return res.status(400).json({ error: "Invalid until" });

    if (startDt && (endDt as Date | null) instanceof Date && endDt! < startDt) {
      return res.status(400).json({ error: "end must be ≥ start" });
    }
