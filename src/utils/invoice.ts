...
import { sendPaymentReceiptEmail } from "./mailerSend"; // adapt if your mailer path differs
import { spaces } from "../config/spaces"; // <--- use spaces client
...
const SIGNED_URL_EXPIRES = Math.min(
  Number(process.env.INVOICE_SIGNED_URL_EXPIRES ?? PRESIGN_MAX),
  PRESIGN_MAX
);
...
export async function generateInvoiceBuffer(
  payment: Payment & { meta?: any; user?: { email?: string } }
) {
  return new Promise<Buffer>((resolve, reject) => {
    ...
    //     .text(`Due: ${dueDate}`, rightX, metaTop + 48, { align: "right" });
      ...
      const billedToName =
        ...
        "Customer";
      const billedToAddr =
        meta.rp_address && Array.isArray(meta.rp_address)
          ? meta.rp_address
          : meta.rp_address
      ...
      if (Array.isArray(meta.items) && meta.items.length > 0) {
        ...
        const desc =
          payment.purpose === "NEW_SUBSCRIPTION"
            ? `Subscription ${meta.planId ?? "plan"} — ${
                meta.seats ?? 1
              } seat(s) — ${meta.period ?? "period"}`
    ...
    //   doc.text(`UPI: ${upi}`, 48, paymentY + 44);
      ...
      doc
        ...
        .fillColor("#666")
        .text(
          `Invoice generated on ${new Date().toISOString().slice(0, 10)}`,
          48,
          770,
...
export async function uploadInvoiceToSpaces(buffer: Buffer, key: string) {
  if (!SPACES_BUCKET) throw new Error("SPACES_BUCKET not configured");
  ...
  const getCmd = new GetObjectCommand({ Bucket: SPACES_BUCKET, Key: key });
  const url = await getSignedUrl(spaces, getCmd, {
    expiresIn: SIGNED_URL_EXPIRES,
  });
  return { key, url };
}

export async function createInvoiceAndNotify(paymentId: string) {
  ...
  // generate PDF buffer
  const pdfBuffer = await generateInvoiceBuffer(payment as any);
  ...
  const metaAfter = (updated.meta as any) ?? {};
  ...
  if (!toEmail) {
    console.warn(
      `createInvoiceAndNotify: no recipient email found for payment ${payment.id}; invoice uploaded but email not sent.`
    );
  ...
  try {
    if (attachPdf) {
      ...
      console.warn(
        `createInvoiceAndNotify: invoice ${updated.id} exceeds ${MAX_ATTACHMENT_BYTES} bytes; sent link instead of attaching.`
      );
      return {
        skipped: false,
        invoiceUrl: url,
