...
export async function getAdminIds(
  ...
  return admins
    ...
    .filter((id: string) => (skip ? id !== skip : true));
}

/** Resolve project head (manager) from project/task/occurrence */
// notify.ts

async function getProjectHeadIds(
  orgPrisma: any,
  ids: {
 ...
 */
export async function notifyCounterparties({
  orgPrisma,
  corePrisma,
  io,
  orgId,
  actor,
  assigneeIds,
  assignedToId, // legacy single-assignee support
  payload,
}: {
  orgPrisma: any;
  corePrisma: any;
  ...
  if (assignedToId) assignees.add(String(assignedToId));
